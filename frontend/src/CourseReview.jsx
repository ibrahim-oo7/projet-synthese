...
export default function CourseReview() {
  ...
  useEffect(() => {
    ...
    axios.get(`http://127.0.0.1:8000/api/center/courses/${id}`, { headers })
  ...
  async function updateStatus(newStatus) {
    const headers = {Authorization: `Bearer ${localStorage.getItem("token")}`};
    const res = await axios.post(`http://127.0.0.1:8000/api/center/courses/${id}/status`,
      { status: newStatus },
      { headers }
    );
                ...
                {lesson?.video ? (
                  <div className="mb-6 rounded-xl overflow-hidden bg-black aspect-video flex items-center justify-center relative group">
                    <div className="absolute inset-0 bg-secondary/10 backdrop-blur-sm flex flex-col items-center justify-center text-white">
                      <video src={`http://127.0.0.1:8000/storage/${lesson.video}`} controls></video> 
                    </div>
                  </div>
                ) : null}
